...
public class Player : MonoBehaviour
{
                                            ...
                                            for (int j = 0; j < equipmentSlots[i].item.buffs.Length; j++)
                                            {
                                                switch (equipmentSlots[i].item.buffs[j].attribute)
                                                {
                                                    case Attributes.Damage:
                                                        mainHand.GetComponent<Gun>().attackDamage = equipmentSlots[i].item.buffs[j].value;
                                                        break;
                                                    ...
                                                    case Attributes.MaxAmmo:
                                                        mainHand.GetComponent<Gun>().maxAmmo = equipmentSlots[i].item.buffs[j].value;
                                                        break;
                                    ...
                                    for (int i = 0; i < equipment.Container.Slots.Length; i++)
                                    {
                                        if (equipmentSlots[i].item.Id == _slot.ItemObject.data.Id)
                                        {
                                            for (int j = 0; j < equipmentSlots[i].item.buffs.Length; j++)
                                            {
                                                ...
                                                {
                                                    case Attributes.Damage:
                                                        offHand.GetComponent<Gun>().attackDamage = equipmentSlots[i].item.buffs[j].value;
                                                        break;
                                                    case Attributes.BulletSpeed:
                                                    ...
                                                    case Attributes.Firerate:
                                                        ...
                                                        break;
                                                    case Attributes.MaxAmmo:
                                                        offHand.GetComponent<Gun>().maxAmmo = equipmentSlots[i].item.buffs[j].value;
                                                        break;
                                                    case Attributes.ReloadTime:
    ...
    public void AttributeModified(Attribute attribute)
    {
        ...
        if (attribute.type == Attributes.Health) setHealth(attribute.value.ModifiedValue);
        if (attribute.type == Attributes.Defense) setDefense(attribute.value.ModifiedValue);
        if (attribute.type == Attributes.Speed) GetComponent<PlayerController>().setSpeed(attribute.value.ModifiedValue);
    }
